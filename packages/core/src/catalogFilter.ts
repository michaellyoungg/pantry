/**
 * The catalog's search and filter behaviour (BL-0020), with no chips attached.
 *
 * Lived inside `apps/web/src/components/CatalogFilters.tsx` until BL-0063 gave
 * the native client a catalog too. Which recipes a selection matches is not a
 * fact about a chip row, and two clients answering "what does `vegan` + `under
 * 30 min` show?" differently is exactly the drift the parity plan's §4 exists to
 * prevent — so the predicate and the derived vocabularies live here and each
 * client draws them however its platform wants.
 *
 * Filtering is client-side over the loaded catalog, which is honest while the
 * seed set is small; `CatalogFilter` is serializable precisely so moving it
 * server-side later is not a rewrite.
 */
import type { Recipe } from "@pantry/types";
import { COOK_TIME_BUCKETS, type CookTimeBucketId, DIET_TAGS } from "./discovery";
...
export const emptyCatalogFilter: CatalogFilter = { query: "", diets: [], cuisines: [] };
 ...
 */
export function cuisinesIn(recipes: Recipe[]): string[] {
...
export function dietsIn(recipes: Recipe[]): string[] {
  const present = new Set(recipes.flatMap((r) => r.tags ?? []));
  return DIET_TAGS.filter((d) => present.has(d));
}

/** Adds or removes one value of a multi-select chip group. */
