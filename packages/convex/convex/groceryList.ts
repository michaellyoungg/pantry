...
//
// Non-destructive merge: lines matching an existing row by item+unit+aisle
// keep their `checked` state (only the quantity is refreshed); new lines are
// inserted unchecked; existing rows absent from the new list are deleted. This
// preserves in-store check-off progress across re-generation.
