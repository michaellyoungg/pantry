...
func TestRecipesUsing_MatchesOwnAndCatalogRecipesRankedByMatchCount(t *testing.T) {
	...
	ctx := t.Context()
	if _, err := store.CreateRecipe(ctx, "user-a", "Spinach & Egg Scramble",
		[]Ingredient{{Quantity: 2, Item: "spinach"}, {Quantity: 3, Item: "eggs"}}); err != nil {
		t.Fatal(err)
	}
	if _, err := store.CreateRecipe(ctx, "user-a", "Toast",
		[]Ingredient{{Quantity: 1, Item: "bread"}}); err != nil {
		t.Fatal(err)
	}
	if _, err := store.CreateRecipe(ctx, CatalogUserID, "Creamed Spinach",
		[]Ingredient{{Quantity: 1, Item: "spinach"}}); err != nil {
		t.Fatal(err)
	}
...
func TestRecipesUsing_DoesNotLeakAnotherUsersRecipes(t *testing.T) {
	srv, store := newTestServer(t)
	if _, err := store.CreateRecipe(t.Context(), "user-b", "Secret Spinach Pie",
		[]Ingredient{{Quantity: 1, Item: "spinach"}}); err != nil {
		t.Fatal(err)
	}
...
func TestRecipesUsing_EmptyItemsYieldsEmptyList(t *testing.T) {
	srv, store := newTestServer(t)
	if _, err := store.CreateRecipe(t.Context(), "user-a", "Toast",
		[]Ingredient{{Quantity: 1, Item: "bread"}}); err != nil {
		t.Fatal(err)
	}
