...
func TestListRecipes_ReturnsOwnerRecipes(t *testing.T) {
	...
	_, _ = store.CreateRecipe(context.Background(), "user-a", "A", nil, nil, nil, nil)
...
func TestGroceryList_AggregatesAcrossRecipeIDs(t *testing.T) {
	...
	a, _ := store.CreateRecipe(ctx, "user-a", "A", []Ingredient{{Quantity: 2, Unit: "cloves", Item: "garlic"}}, nil, nil, nil)
	b, _ := store.CreateRecipe(ctx, "user-a", "B", []Ingredient{{Quantity: 1, Unit: "cloves", Item: "garlic"}}, nil, nil, nil)
...
func TestGroceryList_IncludesCatalogRecipes(t *testing.T) {
	srv, store := newTestServer(t)
	ctx := context.Background()
	own, _ := store.CreateRecipe(ctx, "user-a", "Own", []Ingredient{{Quantity: 1, Unit: "cloves", Item: "garlic"}}, nil, nil, nil)
	cat, _ := store.CreateRecipe(ctx, CatalogUserID, "Catalog", []Ingredient{{Quantity: 2, Unit: "cloves", Item: "garlic"}}, nil, nil, nil)
...
func TestGroceryList_ExcludesOtherUsersRecipes(t *testing.T) {
	srv, store := newTestServer(t)
	ctx := context.Background()
	mine, _ := store.CreateRecipe(ctx, "user-a", "Mine", []Ingredient{{Quantity: 1, Unit: "cloves", Item: "garlic"}}, nil, nil, nil)
	theirs, _ := store.CreateRecipe(ctx, "user-b", "Theirs", []Ingredient{{Quantity: 5, Unit: "cloves", Item: "garlic"}}, nil, nil, nil)
...
func TestDeleteRecipe_RemovesAndReturns204(t *testing.T) {
	srv, store := newTestServer(t)
	rec, _ := store.CreateRecipe(context.Background(), "user-a", "Toast", nil, nil, nil, nil)
...
func TestRecipes_CrossUserReturns404(t *testing.T) {
	srv, store := newTestServer(t)
	rec, err := store.CreateRecipe(context.Background(), "user-a", "Toast", nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("seed: %v", err)
	}
...
func TestListCatalog_ReturnsCatalogOwnedRecipesOnly(t *testing.T) {
	...
	_, _ = store.CreateRecipe(ctx, "user-a", "Mine", nil, nil, nil, nil)
