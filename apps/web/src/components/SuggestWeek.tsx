...
export function SuggestWeek({ items }: { items: readonly PlannedItem[] }) {
  ...
  return (
    ...
    // stored the week. `accept` awaits an add and a schedule per pick and none
    // of those writes is optimistic, so this is the only signal on the card
    // separating "the proposal cleared" from "the week is saved".
    <Card title="Suggest my week" busy={applying}>
